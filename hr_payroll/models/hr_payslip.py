...
class HrPayslip(models.Model):
    ...
    def view_mass_payroll_wizard(self):
        ...
        return {
            'name':"Enviar nóminas de forma masiva por email",
            'type': 'ir.actions.act_window', 
            'view_type': 'form', 
            'view_mode': 'form',
    ...
    def onchange_employee(self):
        ...
        #contracts = self.env['hr.contract'].browse(contract_ids)
        contracts = self.env['hr.contract'].search(['&', ('id', 'in', contract_ids), ('state', '=', 'open')])
        worked_days_line_ids = self.get_worked_day_lines(contracts, date_from, date_to)
...
class HrPayslipRun(models.Model):
    ...
    date_end = fields.Date(string='Date To', required=True, readonly=True,
        ...
        default=lambda self: fields.Date.to_string((datetime.now() + relativedelta(months=+1, day=1, days=-1)).date()))
    credit_note = fields.Boolean(string='Credit Note', readonly=True,
        states={'draft': [('readonly', False)]}, help="If its checked, indicates that all payslips generated from here are refund payslips.")
