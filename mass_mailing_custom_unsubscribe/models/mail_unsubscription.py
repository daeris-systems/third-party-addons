...
class MailUnsubscription(models.Model):
    ...
    unsubscriber_id = fields.Reference(lambda self: self._selection_unsubscriber_id(),"(Un)subscriber",help="Who was subscribed or unsubscribed.",)
    mailing_list_ids = fields.Many2many(comodel_name="mailing.list",string="Mailing lists",help="(Un)subscribed mass mailing lists, if any.",)
    reason_id = fields.Many2one("mail.unsubscription.reason","Reason",ondelete="restrict",help="Why the unsubscription was made.",)
    ...
    metadata = fields.Text(readonly=True, help="HTTP request metadata used when creating this record.")
    ...
    def _check_reason_needed(self):
        ...
        for one in self:
            ...
            if one.action in unsubscription_states and not one.reason_id:
                raise exceptions.ReasonRequiredError(_("Please indicate why are you unsubscribing."))
    ...
    def _check_details_needed(self):
        ...
        for one in self:
            if not one.details and one.details_required:
                raise exceptions.DetailsRequiredError(_("Please provide details on why you are unsubscribing."))
...
class MailUnsubscriptionReason(models.Model):
    ...
    details_required = fields.Boolean(help="Check to ask for more details when this reason is selected.")
